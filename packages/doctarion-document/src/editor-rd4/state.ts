...
import { Cursor } from "../cursor-traversal-rd4";
import { Anchor, Document } from "../document-model-rd4";
import {
  AnchorId,
  InteractorId,
  ReadonlyInteractor,
...
export class EditorState extends WorkingDocument implements ReadonlyEditorState {
  ...
  public get focusedInteractor(): ReadonlyInteractor | undefined {
    if (this.focusedInteractorId !== undefined) {
      return this.interactors.get(this.focusedInteractorId);
    }
    return undefined;
  }

  public anchorToCursor(anchor: AnchorId | ReadonlyWorkingAnchor): Cursor {
    const resolvedAnchor = this.allAnchors.get(typeof anchor === "string" ? anchor : anchor.id);
    if (!resolvedAnchor) {
      throw new EditorError("Unknown anchor");
    }

    const path = this.editorState.lookupPathTo(resolvedAnchor.node);
    if (!path) {
      throw new EditorError("Path to node specified by anchor not found");
    }
    if (resolvedAnchor.graphemeIndex !== undefined) {
      return new Cursor(
        new Path([...path.parts, new PathPart(resolvedAnchor.graphemeIndex)]),
        (resolvedAnchor.orientation as unknown) as CursorOrientation
      );
    }
  ...
  public setFocusedInteractor(id: InteractorId | undefined): void {
    if (id !== undefined) {
      if (this.interactors.get(id) === undefined) {
        throw new EditorError("Invalid interactor id");
      }
    }
